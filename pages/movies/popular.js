...
export default function Popular() {
    ...
    const { list, loading, setList, totalPage } = useGetList(
        {
            url: `${process.env.NEXT_PUBLIC_API_URL}/movie/popular`,
            options: {
                params: {
    ...
    useEffect(() => {
        if (page) {
            router.push(`/movies/popular?page=${page}`, undefined, {
                shallow: true,
            })
        }
    }, [page])
    ...
    return (
        <>
            <Head>
                <meta name="Description" CONTENT="Neftlix - Popular"></meta>
                <title>Neftlix - Popular Moive</title>
